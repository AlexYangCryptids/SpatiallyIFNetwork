...
delta_T_i = 2 # voltage , also ses the paper, to I
...
for step in range(total_step):
    ...
    delta_V_e = -(V_now[:boundary] - VL) / tau_m_e + delta_T_e * np.exp(
        (V_now[:boundary] - VT) / delta_T_e) / tau_m_e + \
                J_ef * synapse_input_ff[:boundary] + J_ee * synapse_input_e + J_ei * synapse_input_i

    delta_V_i = -(V_now[boundary:] - VL) / tau_m_i + delta_T_i * np.exp((V_now[boundary:] - VT) / delta_T_i) / tau_m_i + \
                J_if * synapse_input_ff[boundary:] + J_ie * synapse_input_e + J_ii * synapse_input_i
...
#plt.matshow(3 * spike_record +  poisson_train, aspect='auto', cmap='viridis')
plt.matshow(poisson_train, cmap=plt.cm.Blues, aspect='auto')
#plt.colorbar()
#plt.show()
